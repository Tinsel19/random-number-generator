...
for (i = 0; i <= (x) ; i ++)  {
    randomElement = Math.random().toFixed(digitNum.value).split('.')[1];
    while(randomElement.charAt(0) === '0') {
        randomElement = randomElement.substring(1);
        randomElement = randomElement + "0";
    };
    
    randomElement = randomElement+ '\xa0\xa0\xa0\xa0\xa0 ';
    randomElements = Number(randomElement) ;
...
document.querySelector("#spin").addEventListener ("onLongClick", function () {
    resultScreen.value = "";
    digitcomb = Number(digitNum.value) + 2;
    screen = resultScreen.textContent;
    heading.textContent = "Random " + digitNum.value + " Digit Number Generator";
